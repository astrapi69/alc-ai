...
* the manifest set id (``ki-einsteiger-from-de``) resolves the same
  set as the path basename slug (``ki-einsteiger``),
* ``chunk_lessons()`` splits a lesson list into consecutive groups of at
  most N, is a no-op once split_size reaches the lesson count, and rejects
  a non-positive split_size,
* ``--split-size N`` writes one self-contained file per chunk (each with
  its own ``review_instructions`` copy plus ``part``/``of``/
  ``lesson_count``/``total_lesson_count``), the parts concatenate back to
  the full lesson order, a split_size at or above the lesson count yields
  a single file, and ``--split-size`` combined with ``--out`` is a usage
  error.
"""
...
KNOWN_MANIFEST_ID = "ki-einsteiger-from-de"
...
def test_split_size_writes_one_file_per_chunk_with_correct_lesson_counts(
    ...
    source_lessons = load_source_lessons()
    assert len(source_lessons) == 12  # KNOWN_SLUG fixture assumption

    created_files = run_split_export(tmp_path, 8)
    ...
    assert part_1["lesson_count"] == 8
    assert part_2["lesson_count"] == 4
    assert part_1["total_lesson_count"] == 12
    assert part_2["total_lesson_count"] == 12
